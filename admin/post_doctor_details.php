...
$sql = "SELECT * FROM doctor_details WHERE email_id = '$doctor_email'";
...
if ($result && mysqli_num_rows($result) > 0) {
	...
	if ($doctor_email == $row['email_id']) {
		header("Location: doc_details.php?er=1");
		exit;
	}
}
...
if (isset($_FILES['pic']) && $_FILES['pic']['error'] == 0) {
	...
	if (move_uploaded_file($picture_tmp_name, $target_file)) {
		...
		$query = "INSERT INTO doctor_details(doctor_name, graduation_status, email_id, specialist, fees, gender, address, mobile, status, experience, profile_photo, created_at, update_at)
	...
	$query = "INSERT INTO doctor_details (doctor_name, graduation_status, email_id, specialist, fees, gender, address, mobile, status, experience, created_at, update_at)
